...
public class TelegramChatService : BackgroundService, ITelegramChatService
{
        ...
        try
        {
            ...
            if (isChart && !string.IsNullOrWhiteSpace(chartSql))
            {
                await HandleChartRequestAsync(request.UserId, chartSql!, chartType ?? "bar", chartTitle!, request.ChatId,
                    transactionRepository, aiService, chartService, telegramService, placeholderId, ctsHeartbeat);
                return;
            }
    ...
    private async Task HandleChartRequestAsync(int userId, string chartSql, string chartType, string chartTitle, string chatId,
        ITransactionRepository transactionRepository, IAiService aiService, IChartService chartService, ITelegramService telegramService,
        int placeholderId, CancellationTokenSource ctsHeartbeat)
    {
        try
        {
            // Route through the central, hardened validator: SELECT-only, comments
            // stripped, semicolons rejected, full forbidden-keyword list with word
            // boundaries, mandatory user_id filter, read-only transaction.
            ...
            {
                _logger.LogWarning("Blocked AI-generated SQL for user {UserId}: {Reason}. SQL: {Sql}", userId, ex.Message, chartSql);
                if (placeholderId > 0) await telegramService.EditMessageAsync(userId, placeholderId, "⚠️ <b>Security Alert</b>\n\nThe analytical engine attempted to generate an unauthorized database command. This request has been blocked.", chatId);
                return;
            }
