...
public class TransactionSyncService : ITransactionSyncService
{
    ...
    public async Task SyncTransactionsAsync(int userId, bool silent = false, bool forceCategorizeAll = false, CancellationToken token = default)
    {
        ...
        if (allNewTxs.Count > 0)
        {
            ...
            // Note: We insert them uncategorized here. MediatR handlers will pick them up
            // for AI processing and notifications.
            ...
            if (inserted > 0)
            {
                // PUBLISH EVENT for decoupling: categorization, notifications, etc.
        ...
        else 
        {
            ...
            // Still check for background backlog if we didn't find "new" ones this tick
            await _mediator.Publish(new TransactionsSyncedEvent
            {
                UserId = userId,
